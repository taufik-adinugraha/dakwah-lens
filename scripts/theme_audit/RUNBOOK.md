...
2. **Oversized batches amplify the narration.** → `prepare.py` batches ~**175** (halved after audit#139). The contract in (1) is the primary guard, but size sets the headroom: 64000/349 ≈ 183 output tokens per post vs 64000/175 ≈ 365. Measured in audit#139: full batches passed 3 of 7, halves passed 3 of 3. At 350 a single lapse costs the entire batch.
...
- **Split-batch naming must be `part_NNa` / `part_NNb`** (a letter suffix, never `part_NN_2` or `part_NN_chunk_0`). `aggregate.py` counts `part_NN` and `part_NN<letter>` as real batches and deliberately ignores `part_NN_chunk_N` (subagent-created chunk files). ⚠️ audit#139: a first pass at that glob used a bare `[0-9][0-9]`, which ALSO excluded the lettered halves — a split batch's flags would have been silently dropped from the aggregate with no MISSING warning. Verify with `ls out/flags_*.json` against `aggregate.py`'s reported batch count before applying.
