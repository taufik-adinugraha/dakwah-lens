...
Runs Sunday 18:00 WIB via Celery beat. Pulls the most-recent
`insights_summaries` row (generated by the 04:30 task) and emails it
to every user with `email_digest_opt_in = true`. Each email includes
a one-click unsubscribe link tied to the user's
