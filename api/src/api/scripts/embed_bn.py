"""Embed Al-Bidayah wan-Nihayah (Ibn Kathir) into Qdrant.

Reads `api/data/al-bidayah-wan-nihayah.json` (produced by
`download_bn.py`), embeds each section's Arabic text with OpenAI's
configured embedding model, and upserts into the
`al_bidayah_wan_nihayah` Qdrant collection.
...
added in a later pass. Each section embeds with just its title as
header — no `qism` propagation because the historical-narrative
structure of Ibn Kathir doesn't have a single recurring top-level
divider we can grep.

Chunking
--------
Al-Bidayah sections are short — avg ~1.5k chars, well under the
4000-char chunk threshold. Each section becomes ONE vector. No
windowing needed.

Idempotency
-----------
Point ids are deterministic from `section_id`. Re-running overwrites
existing vectors → no duplicates.

Cost preview
------------
~20M Arabic chars ≈ 14M tokens × $0.130/1M = ~$1.86 one-time at
text-embedding-3-large. ~13,000 vectors total. Use `--dry-run` to
confirm before calling the API.

With the 1M-TPM throttle (5s gap between 50-section batches), runtime
is ~22 min — kicked off once and let to run.

Run
---
    cd api && uv run python -m api.scripts.embed_bn --dry-run
    cd api && uv run python -m api.scripts.embed_bn
"""
...
JSON_PATH = DATA_DIR / "al-bidayah-wan-nihayah.json"
COLLECTION = "al_bidayah_wan_nihayah"
DISPLAY_NAME = "Al-Bidayah wan-Nihayah"

# Sections are short (~1.5k chars ≈ 1k tokens each). 50 per batch =
# ~50k tokens — well under the 300k per-request cap. With the 1M TPM
# org cap we use a 5s gap between batches: 50k tokens × 12 batches/min
# = 600k tokens/min, ~60% of the cap. ~262 batches total, ~22 min run.
EMBED_BATCH = 50
UPSERT_BATCH = 200
...
def _build_inputs(
    ...
    for r in rows:
        ...
        citation_en = (
            f"Al-Bidayah wan-Nihayah — {title}" if title else "Al-Bidayah wan-Nihayah"
        )
...
def _embed(
    ...
    if not JSON_PATH.exists():
        raise SystemExit(
            ...
            "`uv run python -m api.scripts.download_bn` first."
        )
    rows = json.loads(JSON_PATH.read_text(encoding="utf-8"))
    texts, payloads, ids = _build_inputs(rows)
    if not texts:
        log.warning("embed_bn.empty")
        return 0, 0
    ...
    log.info("embed_bn.done", embedded=len(points), tokens=tokens_used)
