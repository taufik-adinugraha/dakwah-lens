"""Embed Ash-Shama'il al-Muhammadiyyah (al-Tirmidhi) into Qdrant.

Reads `api/data/syamail-muhammadiyyah.json` (produced by
`download_syamail.py`), embeds each chapter's Arabic text with OpenAI's
configured embedding model, and upserts into the
`syamail_muhammadiyyah` Qdrant collection.

What we embed
-------------
Arabic only. ~55 chapters covering the Prophet's appearance, manners,
conduct. Longer chapters (>8k chars) chunk into multiple vectors at
the same 8k cap calibrated on Fath al-Mu'in.

Run
---
    cd api && uv run python -m api.scripts.embed_syamail --dry-run
    cd api && uv run python -m api.scripts.embed_syamail
"""
...
JSON_PATH = DATA_DIR / "syamail-muhammadiyyah.json"
COLLECTION = "syamail_muhammadiyyah"
DISPLAY_NAME = "Ash-Shama'il al-Muhammadiyyah"
...
def _build_inputs(
    ...
    for r in rows:
        ...
        if qism and title and qism != title:
            citation_en = f"Ash-Shama'il al-Muhammadiyyah — {qism} / {title}"
        elif title:
            citation_en = f"Ash-Shama'il al-Muhammadiyyah — {title}"
        else:
            citation_en = "Ash-Shama'il al-Muhammadiyyah"
...
def _embed(
    ...
    if not JSON_PATH.exists():
        raise SystemExit(
            ...
            "`uv run python -m api.scripts.download_syamail` first."
        )
    rows = json.loads(JSON_PATH.read_text(encoding="utf-8"))
    texts, payloads, ids = _build_inputs(rows)
    if not texts:
        log.warning("embed_syamail.empty")
        return 0, 0
    ...
    for i in range(0, len(texts), EMBED_BATCH):
        ...
        record_usage(
            provider="openai",
            operation="corpus_embed_syamail",
    ...
    log.info("embed_syamail.done", embedded=len(points), tokens=tokens_used)
...
def _dry_run() -> None:
    ...
    if not JSON_PATH.exists():
        ...
        print("Run download_syamail first.")
        return
    rows = json.loads(JSON_PATH.read_text(encoding="utf-8"))
    ...
    print(f"Vectors      : {len(texts)} (chunked at {MAX_CHARS_PER_VECTOR:,} chars)")
