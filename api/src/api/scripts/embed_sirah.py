"""Embed As-Sirah an-Nabawiyyah (Ibn Hisham) into Qdrant.

Reads `api/data/sirah-ibn-hisham.json` (produced by `download_sirah.py`),
embeds each chunk's Arabic text with OpenAI's configured embedding
model, and upserts into the `sirah_ibn_hisham` Qdrant collection.

What we embed
-------------
Arabic only. The download script chunks by 3-page windows so most
sections fit a single 8k vector; the few that overflow chunk again
at the same 8k cap.

Run
---
    cd api && uv run python -m api.scripts.embed_sirah --dry-run
    cd api && uv run python -m api.scripts.embed_sirah
"""
...
JSON_PATH = DATA_DIR / "sirah-ibn-hisham.json"
COLLECTION = "sirah_ibn_hisham"
DISPLAY_NAME = "Sirah Ibn Hisham"
...
def _build_inputs(
    ...
    for r in rows:
        ...
        if qism and title and qism != title:
            citation_en = f"Sirah Ibn Hisham — {qism} / {title}"
        elif title:
            citation_en = f"Sirah Ibn Hisham — {title}"
        else:
            citation_en = "Sirah Ibn Hisham"
...
def _embed(
    ...
    if not JSON_PATH.exists():
        raise SystemExit(
            ...
            "`uv run python -m api.scripts.download_sirah` first."
        )
    rows = json.loads(JSON_PATH.read_text(encoding="utf-8"))
    texts, payloads, ids = _build_inputs(rows)
    if not texts:
        log.warning("embed_sirah.empty")
        return 0, 0
    ...
    log.info("embed_sirah.done", embedded=len(points), tokens=tokens_used)
...
def _dry_run() -> None:
    ...
    if not JSON_PATH.exists():
        ...
        print("Run download_sirah first.")
        return
    rows = json.loads(JSON_PATH.read_text(encoding="utf-8"))
