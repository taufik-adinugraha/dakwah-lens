"""Embed Nashaihul Ibad (Nawawi al-Bantani's commentary on Ibn Hajar's
Munabbihat) into Qdrant.

Reads `api/data/nashaihul-ibad.json` (produced by `download_nashaih.py`),
embeds each section's Arabic text with OpenAI's configured embedding
model, and upserts into the `nashaihul_ibad` Qdrant collection.

What we embed
-------------
Arabic only for now (decision 2026-06-08). Pre-chunked by the download
script into ~2-3k char sections at paragraph boundaries — every chunk
is one or a few narrations / numbered groups of advice.

Cost preview
------------
~77k Arabic chars ≈ 55k tokens × $0.130/1M = ~$0.007 one-time. Tiny —
the kitab is short (~62 pages). 34 vectors total. Use `--dry-run` to
confirm.

Run
---
    cd api && uv run python -m api.scripts.embed_nashaih --dry-run
    cd api && uv run python -m api.scripts.embed_nashaih
"""
...
JSON_PATH = DATA_DIR / "nashaihul-ibad.json"
COLLECTION = "nashaihul_ibad"
DISPLAY_NAME = "Nashaihul Ibad"

# 34 sections fit comfortably in one batch — no throttling needed for
# this kitab. Single batch, single request, ~5 sec total.
EMBED_BATCH = 50
UPSERT_BATCH = 200
...
def _build_inputs(
    ...
    for r in rows:
        ...
        citation_en = (
            f"Nashaihul Ibad — {title}" if title else "Nashaihul Ibad"
        )
...
def _embed(
    ...
    if not JSON_PATH.exists():
        raise SystemExit(
            ...
            "`uv run python -m api.scripts.download_nashaih` first."
        )
    rows = json.loads(JSON_PATH.read_text(encoding="utf-8"))
    texts, payloads, ids = _build_inputs(rows)
    if not texts:
        log.warning("embed_nashaih.empty")
        return 0, 0
    ...
    for i in range(0, len(texts), EMBED_BATCH):
        ...
        record_usage(
            provider="openai",
            operation="corpus_embed_nashaih",
    ...
    log.info("embed_nashaih.done", embedded=len(points), tokens=tokens_used)
...
def _dry_run() -> None:
    ...
    if not JSON_PATH.exists():
        ...
        print("Run download_nashaih first.")
        return
    rows = json.loads(JSON_PATH.read_text(encoding="utf-8"))
