"""Embed Adab al-'Alim wa al-Muta'allim (Ibn Jama'ah's Tadhkirat) into Qdrant.

Reads `api/data/adab-alim-mutaallim.json` (produced by `download_adab.py`),
embeds each section's Arabic text with OpenAI's configured embedding
model, and upserts into the `adab_alim_mutaallim` Qdrant collection.

What we embed
-------------
Arabic only for now. Each TOC anchor becomes a section spanning the
pages between it and the next anchor. Sections >8k chars chunk into
multiple vectors (same 8k cap as embed_fmuin / embed_fqarib — Arabic
with inline harakat tokenises ~1 char/token in dense patches).

Cost preview
------------
TBD by --dry-run. Expected ~150k chars (204 pages × ~750 chars/page)
≈ 50k tokens × $0.130/1M = ~$0.007.

Run
---
    cd api && uv run python -m api.scripts.embed_adab --dry-run
    cd api && uv run python -m api.scripts.embed_adab
"""
...
JSON_PATH = DATA_DIR / "adab-alim-mutaallim.json"
COLLECTION = "adab_alim_mutaallim"
DISPLAY_NAME = "Adab al-'Alim wa al-Muta'allim"
...
def _build_inputs(
    ...
    for r in rows:
        ...
        if qism and title and qism != title:
            citation_en = f"Adab al-'Alim wa al-Muta'allim — {qism} / {title}"
        elif title:
            citation_en = f"Adab al-'Alim wa al-Muta'allim — {title}"
        else:
            citation_en = "Adab al-'Alim wa al-Muta'allim"
...
def _embed(
    ...
    if not JSON_PATH.exists():
        raise SystemExit(
            ...
            "`uv run python -m api.scripts.download_adab` first."
        )
    rows = json.loads(JSON_PATH.read_text(encoding="utf-8"))
    texts, payloads, ids = _build_inputs(rows)
    if not texts:
        log.warning("embed_adab.empty")
        return 0, 0
    ...
    log.info("embed_adab.done", embedded=len(points), tokens=tokens_used)
