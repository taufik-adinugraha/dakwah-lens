"""Embed Fiqh as-Sunnah (Sayyid Sabiq) into Qdrant.

Reads `api/data/fiqh-as-sunnah.json` (produced by `download_fs.py`),
embeds each section's Arabic text with OpenAI's configured embedding
model, and upserts into the `fiqh_as_sunnah` Qdrant collection.

What we embed
-------------
Arabic only for now (decision 2026-06-08). 866 sections at ~1k chars
each — well below the 300k tokens/request cap, so larger batches are
safe (EMBED_BATCH=50). Cross-lingual retrieval from ID/EN queries
works via text-embedding-3-large's multilingual training.

Cost preview
------------
~900k Arabic chars ≈ 640k tokens × $0.130/1M = ~$0.083 one-time.
Use `--dry-run` to confirm before spending.

Run
---
    cd api && uv run python -m api.scripts.embed_fs --dry-run
    cd api && uv run python -m api.scripts.embed_fs
"""
...
JSON_PATH = DATA_DIR / "fiqh-as-sunnah.json"
COLLECTION = "fiqh_as_sunnah"
DISPLAY_NAME = "Fiqh as-Sunnah"

# Sections are short (~1k chars / ~700 tokens) so 50 per batch ≈ 35k
# tokens — fits comfortably under the 300k tokens/request cap and the
# 1M TPM org cap with our 5s throttle.
EMBED_BATCH = 50
UPSERT_BATCH = 200
...
def _build_inputs(
    ...
    for queries that name the broad fiqh kitab (qism = الصلاة, الزكاة, ...)
    or a specific chapter title."""
    texts: list[str] = []
    ...
    for r in rows:
        ...
        # Citation rendered in chip: prefer qism + chapter; bare title
        # alone misses the kitab context the reader needs.
        if qism and title and qism != title:
            citation_en = f"Fiqh as-Sunnah — {qism} / {title}"
        elif title:
            citation_en = f"Fiqh as-Sunnah — {title}"
        else:
            citation_en = "Fiqh as-Sunnah"
...
def _embed(
    ...
    if not JSON_PATH.exists():
        raise SystemExit(
            ...
            "`uv run python -m api.scripts.download_fs` first."
        )
    rows = json.loads(JSON_PATH.read_text(encoding="utf-8"))
    texts, payloads, ids = _build_inputs(rows)
    if not texts:
        log.warning("embed_fs.empty")
        return 0, 0
    ...
    log.info("embed_fs.done", embedded=len(points), tokens=tokens_used)
