"""Embed Hayat as-Sahabah (Muhammad Yusuf al-Kandahlawi) into Qdrant.

See `download_hs.py` header for source attribution and the rights note
recorded with the project owner's 2026-06-08 decision to proceed.

Reads `api/data/hayat-as-sahabah.json` (produced by `download_hs.py`),
embeds each section's Arabic text, and upserts into the
`hayat_as_sahabah` Qdrant collection. Same 8k chunk cap as the other
AR-only kitabs.

Run
---
    cd api && uv run python -m api.scripts.embed_hs --dry-run
    cd api && uv run python -m api.scripts.embed_hs
"""
...
JSON_PATH = DATA_DIR / "hayat-as-sahabah.json"
COLLECTION = "hayat_as_sahabah"
DISPLAY_NAME = "Hayat as-Sahabah"
...
def _build_inputs(
    ...
    for r in rows:
        ...
        if qism and title and qism != title:
            citation_en = f"Hayat as-Sahabah — {qism} / {title}"
        elif title:
            citation_en = f"Hayat as-Sahabah — {title}"
        else:
            citation_en = "Hayat as-Sahabah"
...
def _embed(
    ...
    if not JSON_PATH.exists():
        raise SystemExit(
            ...
            "`uv run python -m api.scripts.download_hs` first."
        )
    rows = json.loads(JSON_PATH.read_text(encoding="utf-8"))
    texts, payloads, ids = _build_inputs(rows)
    if not texts:
        log.warning("embed_hs.empty")
        return 0, 0
    ...
    log.info("embed_hs.done", embedded=len(points), tokens=tokens_used)
