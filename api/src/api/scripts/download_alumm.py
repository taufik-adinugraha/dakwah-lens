"""Download + parse Al-Umm (Imam al-Shafi'i) for kitab corpus.

Source: Shamela EPUB at https://old.shamela.ws/epubs/016/1655.epub
Edition: Dar al-Fikr (ط الفكر). Author Muhammad ibn Idris al-Shafi'i
(d. 204H/820 CE), founder of the Shafi'i madhhab — public domain.
...
Structure
---------
The EPUB TOC has ~1,900 nested entries — top-level `كتاب X` headings
(Book of Purification, Book of Prayer, ...), each with many `باب`
chapters and `فصل` sub-sections beneath. We walk the TOC top-down and
use any `كتاب X` label as the kitab-level context propagated into all
sections below it — same pattern as download_bidayat.py uses for `قسم`.

Output
------
`api/data/al-umm.json` — array of records:
  { section_id, qism, title, ar, char_count }
ready for `embed_alumm.py` to chunk + embed.

Run
---
    cd api && uv run python -m api.scripts.download_alumm
"""
...
EPUB_URL = "https://old.shamela.ws/epubs/016/1655.epub"
DATA_DIR = Path(__file__).resolve().parents[3] / "data"
OUTPUT_PATH = DATA_DIR / "al-umm.json"

# `كتاب X` prefixes Al-Umm's top-level fiqh divisions ("Kitab al-Tahara",
# "Kitab al-Salah", ...) — equivalent to Bidayat's `قسم`. Sections AFTER
# one of these labels and BEFORE the next inherit it as kitab context.
# The `\S` guard skips the lone string "كتاب" or "كتاب الأم..." (book
# title) — only `كتاب <word>` divider headings match.
_QISM_RE = re.compile(r"^كتاب\s+\S")
