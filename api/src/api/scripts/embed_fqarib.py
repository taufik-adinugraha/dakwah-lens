"""Embed Fath al-Qarib al-Mujib (Ibn Qasim al-Ghazzi) into Qdrant.

Reads `api/data/fath-al-qarib.json` (produced by `download_fqarib.py`),
embeds each section's Arabic text with OpenAI's configured embedding
model, and upserts into the `fath_al_qarib` Qdrant collection.

What we embed
-------------
Arabic only for now (decision 2026-06-08). 212 sections averaging
~1.2k chars each — most fit in a single 8k-char vector; only a handful
will chunk.

Cost preview
------------
~256k Arabic chars ≈ 90k tokens × $0.130/1M = ~$0.012 one-time.
Use `--dry-run` to confirm before spending.

Run
---
    cd api && uv run python -m api.scripts.embed_fqarib --dry-run
    cd api && uv run python -m api.scripts.embed_fqarib
"""
...
JSON_PATH = DATA_DIR / "fath-al-qarib.json"
COLLECTION = "fath_al_qarib"
DISPLAY_NAME = "Fath al-Qarib"

EMBED_BATCH = 30
UPSERT_BATCH = 200

# Same 8k cap empirically calibrated on Fath al-Mu'in — Arabic with
# inline harakat tokenises at ~1 char/token in dense patches, so 12k
# overshoots the 8192-token API limit while 8k leaves headroom for the
# qism/title header we prepend.
...
def _build_inputs(
    ...
    for r in rows:
        ...
        if qism and title and qism != title:
            citation_en = f"Fath al-Qarib — {qism} / {title}"
        elif title:
            citation_en = f"Fath al-Qarib — {title}"
        else:
            citation_en = "Fath al-Qarib"
...
def _embed(
    ...
    if not JSON_PATH.exists():
        raise SystemExit(
            ...
            "`uv run python -m api.scripts.download_fqarib` first."
        )
    rows = json.loads(JSON_PATH.read_text(encoding="utf-8"))
    texts, payloads, ids = _build_inputs(rows)
    if not texts:
        log.warning("embed_fqarib.empty")
        return 0, 0
    ...
    log.info("embed_fqarib.done", embedded=len(points), tokens=tokens_used)
...
def _dry_run() -> None:
    ...
    if not JSON_PATH.exists():
        ...
        print("Run download_fqarib first.")
        return
    rows = json.loads(JSON_PATH.read_text(encoding="utf-8"))
