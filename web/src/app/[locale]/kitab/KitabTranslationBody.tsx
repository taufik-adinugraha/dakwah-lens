...
const COMPONENTS: Components = {
  ...
  ul: ({ children }) => (
    <ul className="mt-2 ml-5 list-disc space-y-0.5 text-sm text-ink-muted">
      {children}
    </ul>
  ),
  ol: ({ children }) => (
    <ol className="mt-2 ml-5 list-decimal space-y-0.5 text-sm text-ink-muted">
      {children}
    </ol>
  ),
  li: ({ children }) => (
  ...
  blockquote: ({ children }) => (
    <blockquote className="my-2 border-l-2 border-forest/40 bg-forest-tint/40 py-1 pl-3 italic text-ink-muted">
      {enhanceArabic(children as ReactNode)}
    </blockquote>
  ),
